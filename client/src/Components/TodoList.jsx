...
const TodoList = ({ todos, userid }) => {
    ...
    const deleteTodo = async (todoid) => {
        await fetch(`http://localhost:8000/app/deletetodo/${userid}/${todoid}`, {
            method: 'delete',
            credentials: "include"
    ...
    // Eadit Todo
    const eaditTodo = (index) => {
        console.log(index);
        let updateTodoData = prompt("Enter Updated todo : ")
        fetch(`http://localhost:8000/app/updatetodo/${userid}/${index}`, {
            method: 'put',
            credentials: "include",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ todoUpdate: updateTodoData })
        })
    }
